...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print('Fetching {0} schedule.'.format(options["year"]))
        ...
        for el in schedule_json["league"]["standard"]:
            if not Game.objects.filter(game_id = el["gameId"]).exists():
                ...
                if update==0:
                    print('Initializing {0} schedule.'.format(options["year"]))
                    update=1

                Game.objects.create(season=options["year"],game_id = el["gameId"] , game_utc = pytz.utc.localize(datetime.strptime(el["startTimeUTC"], '%Y-%m-%dT%H:%M:%S.%fZ')), game_date = el["startDateEastern"] , stage = el["seasonStageId"] , tricode = el["gameUrlCode"].split('/')[1] , h_team = el["hTeam"]["teamId"] , a_team = el["vTeam"]["teamId"] , jumpers = [] , all_first_scorers = [] , all_first_shooters = [], jump_win = 0, team_first_score = 0, player_first_score = 0, last_update = TODAY_UTC)
            else:
                g=Game.objects.get(game_id = el["gameId"])
                gt=pytz.utc.localize(datetime.strptime(el["startTimeUTC"], '%Y-%m-%dT%H:%M:%S.%fZ'))
                if g.game_utc !=gt:
                    print('{0} game (gameID: {1}) rescheduled from {2} to {3}.'.format(g.tricode,g.game_id,g.game_time,gt))
                    update=1
                    g.game_time = gt
